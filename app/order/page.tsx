...
import { menuItems, milkChoices, syrupChoices, tempChoices, categoryLabels, type Category } from '@/lib/menu-data'
...
export default function OrderPage() {
  ...
  const handleAdd = () => {
    ...
    add(selectedItem, {
      ...
      note:  [
        selectedItem.syrupOptions && syrup !== 'No syrup' ? syrup : '',
        note,
      ...
      <div className="pt-14 min-h-screen bg-cream flex items-center justify-center px-5">
        <div className="text-center max-w-sm">
          ...
          <p className="text-sm text-ink font-medium mb-6">
            Ready in about <span className="text-sage">10–15 minutes.</span>
          </p>
                ...
                return (
                  <div key={item.id}
                    className={`bg-cream rounded-sm border transition-all overflow-hidden ${
                      isSel ? 'border-sage' : 'border-bone hover:border-clay'
                    }`}>
                    <button
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {item.tags?.includes('house special') && <span className="pill bg-rust/10 text-rust">house special</span>}
                        </div>
                      ...
                      <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
                        ...
                        {isSel ? <ChevronUp size={14} className="text-sage"/> : <ChevronDown size={14} className="text-clay"/>}
                      </div>
                    </button>
                            ...
                            <div className="flex flex-col gap-1.5">
                              {syrupChoices.map(s => (
                                <button key={s} onClick={() => setSyrup(s)}
                                  className={`py-2 px-2.5 text-xs font-medium rounded-sm border transition-colors text-left ${
                                    syrup === s ? 'bg-ink text-cream border-ink' : 'border-bone text-mist bg-cream hover:border-clay'
                                  }`}>{s}</button>
                              ))}
                            </div>
                          </div>
